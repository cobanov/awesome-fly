...
- [FLYT3](https://github.com/seanphan/flyt3) by **seanphan** - Playable tic-tac-toe against a full 166,700-neuron MaleCNS simulation (fixed wiring, REINFORCE-trained descending+VNC readout), plus a fire-watch mode that classifies satellite wildfire-damage tiles through the same frozen circuit (60.8% val, 5-class) with live connectome wave and region-lesioning controls; reports a negative comparison where a pooled-pixel MLP beats the circuit decoder (85.7%). Self-hosted demo, LAN only.
