...
- [fly-flappy](https://github.com/ns2250225/fly-flappy) by **ns2250225** - Browser Flappy Bird experiment running the full MaleCNS v1.0 graph at 50 Hz with engineered visual inputs and either raw DNp01 decoding or a trainable logistic action readout. **Full retained graph; interface and readout are artificial.** The source repository currently has no separate license file.
...
- [fly-swing](https://github.com/JHC56/fly-swing) by **JHC56** - A MuJoCo fruit fly that web-swings through a random obstacle course like Spider-Man. There is no dodge code. The escape is fired by the real giant fiber circuit from MaleCNS (LC4/LPLC2 -> DNp01, LIF), and the whole brain runs live on screen. The fly picks where to swing with a kNN threat memory built on whitetree. It reaches the goal on 20 out of 20 random courses. **Circuit subset drives the fly; full retained graph is display only.**
- [The Fly's Table](https://github.com/WilliamJones/fly-blackjack) by **WilliamJones** - Heads-up blackjack against a MaleCNS mushroom-body model: 682 projection neurons, 1,200 Kenyon cells, 97 output neurons and 332 dopaminergic neurons with measured wiring, learning at the table through a project-defined dopamine-gated depression rule. One seat, a queue, spectators, and a brain shared through Firestore that persists between players. The same repository holds an 80-neuron Dino controller, a Flybody walker, and odour-conditioning experiments, each with silenced-circuit or dopamine-cut controls. **Circuit subset; only Kenyon-cell-to-output synaptic gains change.** Reports its own results: 30,000 solo hands move the fly from a -45% to a -16% winrate with 52% agreement with basic strategy, disconnecting dopamine removes the gain, and a poker attempt is documented as a negative result. [Play](https://fly-blackjack.vercel.app/).
- [Fly-NAF](https://github.com/ArtyMend07/Fly-NAF) by **ArtyMend07** - Five Nights at Freddy's 1 played by the whole FlyWire v783 graph, 138,639 LIF neurons with their measured weights and nothing trained in the loop. DNp01 slams the doors, accumulated DNp09 potential raises the camera tablet, and raising it drives the GABAergic inhibitor clusters so the fly goes blind while on cameras. Builds on [fly-brain](https://github.com/eonsystemspbc/fly-brain), GPL v3. **Full retained graph; no trained readout.** A starvation timer forces a look at any hallway unwatched for thirty seconds, counted separately in the session report. Windows only, no playable demo. Best run reached 4 AM on night 2. [Recording](https://www.youtube.com/watch?v=4UNPlA-YJtw).
...
- [fly-odor-onoff](https://github.com/yukincom/fly-odor-onoff) by **yukincom** - Reproducible odor ON/OFF intervention experiments using [Fly Brain Minecraft](https://github.com/blendi-remade/fly-brain-minecraft)'s filtered MaleCNS LIF graph. Includes input-pathway audits and four-PN DM1/VA2 responses under externally scheduled OFF-only input blocking. **Fixed circuit; no biological validation.** [Methods and results](https://github.com/yukincom/fly-odor-onoff/blob/main/experiments/brain-four-pn-readout/README_eng.md).
...
- [BioReservoir](https://github.com/igdigitallab/bioreservoir) by **igdigitallab** - Whole retained MaleCNS graph run as a fixed reservoir for visitor questions: a text embedding sets Poisson rates on real sensory populations, the full brain runs 250 ms of LIF per trial, and the answer is the lateral bias across descending neurons after subtracting each brain's intrinsic handedness measured on neutral reference sentences. Reproduces Shiu et al.'s sugar -> MN9 response; the female BANC connectome fails the equivalent lateral check and that failure is published rather than dropped. **Not trained on outcomes and makes no forecasting claim - entertainment and research. Per-run measurements against rewired-connectome, no-connectome and random-graph controls are published in the repository.** [Live page](https://fly.igdigi.com) · [results](https://github.com/igdigitallab/bioreservoir/blob/main/experiments/001-fly-oracle/RESULTS.md). Submitted by the project's own team.
