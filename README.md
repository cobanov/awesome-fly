...
- [BioReservoir](https://github.com/igdigitallab/bioreservoir) by **igdigitallab** - Whole retained MaleCNS graph run as a fixed reservoir for visitor questions: a text embedding sets Poisson rates on real sensory populations, the full brain runs 250 ms of LIF per trial, and the answer is the lateral bias across descending neurons after subtracting each brain's intrinsic handedness measured on neutral reference sentences. Reproduces Shiu et al.'s sugar -> MN9 response; the female BANC connectome fails the equivalent lateral check and that failure is published rather than dropped. **Measured answers sit close to a coin flip (mean |p(yes) - 0.5| = 0.002 across 33 questions, 429 runs including no-brain, rewired, random-graph and seeded-coin controls) - entertainment and research, not a forecast.** [Live page](https://fly.igdigi.com) · [reported results](https://fly.igdigi.com/data). Submitted by the project's own team.
